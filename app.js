(() => {
  ...
  async function apiRequest(action, payload = {}, token = '', options = {}) {
    ...
    const timeout = setTimeout(() => controller.abort(), options.timeoutMs || Number(C.API_TIMEOUT_MS || 90000));
  ...
  async function initializeAccess() {
    ...
    try {
      ...
      state.serverVersion = String(result.serverVersion || '');
      state.audioMode = String(result.audioMode || '');
      if (state.audioMode !== 'DIRECT_REPRODUCIBLE') {
        throw new Error('La página y Apps Script no tienen la misma versión. Actualice la implementación de Apps Script y vuelva a publicar GitHub Pages.');
      }
      showView('loginView');
  ...
  function metricsKey() {
    return state.sessionId ? `metrics|${state.sessionId}` : 'metrics|none';
  }

  async function persistMetrics() {
    if (!state.sessionId) return;
    try {
      await dbPut('meta', { key: metricsKey(), throughputBps: state.throughputBps, uploadFailures: state.uploadFailures, requiredOpsCreated: state.requiredOpsCreated, requiredOpsDone: state.requiredOpsDone, fullOpsDone: state.fullOpsDone, lastQueueError: state.lastQueueError, lastQueueErrorAt: state.lastQueueErrorAt });
    } catch (_) {}
  }
  ...
  async function restoreMetrics() {
    ...
    state.requiredOpsCreated = 0;
    state.requiredOpsDone = 0;
    state.fullOpsDone = 0;
    state.lastQueueError = '';
    state.lastQueueErrorAt = 0;
    try {
      const item = await dbGet('meta', metricsKey());
      if (item) {
        ...
        state.lastQueueError = String(item.lastQueueError || '');
        state.lastQueueErrorAt = Number(item.lastQueueErrorAt || 0);
      }
    } catch (_) {}
  }
  ...
  async function login() {
    ...
    try {
      ...
      if (String(result.policy?.audioMode || '') !== 'DIRECT_REPRODUCIBLE') {
        throw new Error('Apps Script está ejecutando una versión anterior. Actualice la implementación antes de continuar.');
      }
      ...
      await openDb();
      const localFullItems = (await dbGetAll('encrypted')).filter((item) => item.sessionId === state.sessionId && item.kind === 'full');
      state.fullSequence = localFullItems.reduce((max, item) => Math.max(max, Number(item.sequence || 0)), 0);
  ...
  async function startQuestionSegment_(context) {
    ...
    recorder.onstop = () => {
      ...
      if (context.active) {
        state.questionChain = state.questionChain.then(() => startQuestionSegment_(context));
      }
    };
    ...
    }, Number(C.QUESTION_AUDIO_SEGMENT_MS || 60000));
  }
  ...
  function queueRecordId(item) {
    const payload = item.payload || {};
    if (item.action === 'submitAnswer') return `answer|${state.sessionId}|${payload.questionId}|${payload.revision}`;
    if (item.action === 'uploadQuestionAudioPart') return `qaudio|${state.sessionId}|${payload.questionId}|${payload.revision}|${payload.part}`;
    if (item.action === 'uploadFullAudioPart') return `fullaudio|${state.sessionId}|${payload.sequence}`;
    return `${item.action}|${state.sessionId}|${crypto.randomUUID()}`;
  }

  async function enqueue(item) {
    const id = item.id || queueRecordId(item);
    const existing = await dbGet('queue', id);
    if (existing) return existing.id;
    const record = { id, sessionId: state.sessionId, createdAt: Date.now(), tries: 0, nextAttemptAt: 0, lastError: '', ...item };
  ...
  async function serializeQueueItem(item) {
    ...
    if (item.encryptedId) {
      ...
      if (Number(encrypted.plainBytes || 0) > 0 && Number(encrypted.plainBytes) !== plaintext.byteLength) {
  ...
  function retryDelayMs(tries) {
    const ceiling = Number(C.QUEUE_RETRY_MAX_SECONDS || 300) * 1000;
    const base = Math.min(ceiling, 1500 * 2 ** Math.min(tries, 8));
    return Math.min(ceiling, base + Math.floor(Math.random() * 750));
  }

  function scheduleQueueWake(delayMs) {
    if (state.queueWakeTimer) clearTimeout(state.queueWakeTimer);
    if (!state.sessionId || state.completed) return;
    state.queueWakeTimer = setTimeout(() => {
      state.queueWakeTimer = null;
      processQueue();
    }, Math.max(500, Number(delayMs || 1000)));
  }

  function fatalQueueError(message) {
    return /sesión de seguridad|sesión venció|sesión no está activa|servicio está temporalmente suspendido|entorno no está autorizado/i.test(String(message || ''));
  }

  async function processQueue() {
    if (state.queueRunning || !navigator.onLine || !state.token) return;
    state.queueRunning = true;
    if (state.queueWakeTimer) {
      clearTimeout(state.queueWakeTimer);
      state.queueWakeTimer = null;
    }
    try {
      while (navigator.onLine) {
        let items = (await dbGetAll('queue')).filter((x) => x.sessionId === state.sessionId);
        if (!items.length) {
          const queued = await queueEligibleFullItems();
          if (!queued) break;
          items = (await dbGetAll('queue')).filter((x) => x.sessionId === state.sessionId);
          if (!items.length) break;
        }

        const now = Date.now();
        const eligible = items
          .filter((x) => Number(x.nextAttemptAt || 0) <= now)
          .sort((a, b) => a.priority - b.priority || a.createdAt - b.createdAt);

        if (!eligible.length) {
          const nextAt = Math.min(...items.map((x) => Number(x.nextAttemptAt || now + 1000)).filter((x) => x > now));
          scheduleQueueWake(Math.max(500, nextAt - now));
          break;
        }
        ...
        try {
          ...
          const isAudio = item.action === 'uploadQuestionAudioPart' || item.action === 'uploadFullAudioPart';
          await apiRequest(item.action, payload, state.token, {
            timeoutMs: isAudio ? Number(C.AUDIO_UPLOAD_TIMEOUT_MS || 240000) : Number(C.API_TIMEOUT_MS || 90000)
          });
          ...
          item.nextAttemptAt = Date.now() + retryDelayMs(item.tries);
          state.lastQueueError = item.lastError;
          state.lastQueueErrorAt = Date.now();
          await dbPut('queue', item);
          await persistMetrics();
          if (fatalQueueError(item.lastError)) {
            setMessage($('syncText'), item.lastError, 'error');
            break;
          }
          if (!navigator.onLine) break;
          // Importante: no bloqueamos las demás respuestas detrás de un audio fallido.
          // El elemento fallido vuelve a intentarse después de su espera exponencial.
        }
        await updateQueueUi();
        await updateClosingProgress();
        await new Promise((resolve) => setTimeout(resolve, Number(C.QUEUE_MIN_INTERVAL_MS || 700)));
      }
    } finally {
      state.queueRunning = false;
      await updateQueueUi();
      const pending = (await dbGetAll('queue')).filter((x) => x.sessionId === state.sessionId);
      if (pending.length && navigator.onLine) {
        const now = Date.now();
        const nextAt = Math.min(...pending.map((x) => Math.max(now + 500, Number(x.nextAttemptAt || now + 500))));
        scheduleQueueWake(nextAt - now);
      }
    }
  }
  ...
  async function queueEligibleFullItems() {
    ...
    const pendingBytes = items.reduce((sum, x) => sum + Number(x.plainBytes || x.bytes || 0), 0);
  ...
  async function requiredQueueStats() {
    ...
    return {
      count: queue.length,
      bytes: queue.reduce((sum, x) => sum + Number(x.bytes || 0), 0),
      failed: queue.filter((x) => Number(x.tries || 0) > 0).length,
      lastError: queue.sort((a, b) => Number(b.nextAttemptAt || 0) - Number(a.nextAttemptAt || 0))[0]?.lastError || ''
    };
  }

  async function requiredProgressStats() {
    const [answers, encrypted, queue] = await Promise.all([
      dbGetAll('answers'),
      dbGetAll('encrypted'),
      dbGetAll('queue')
    ]);
    const answerCount = answers.filter((x) => x.sessionId === state.sessionId).length;
    const audioCount = encrypted.filter((x) => x.sessionId === state.sessionId && x.kind === 'question').length;
    const pending = queue.filter((x) => x.sessionId === state.sessionId && x.required).length;
    const total = Math.max(0, answerCount + audioCount);
  ...
  async function fullLocalStats() {
    ...
    return {
      ...
      pendingBytes: items.filter((x) => !x.uploaded).reduce((sum, x) => sum + Number(x.plainBytes || x.bytes || 0), 0)
    };
  }
  ...
  async function updateQueueUi() {
    ...
    if ($('syncText')) {
      ...
      if (!navigator.onLine) {
        ...
        const retryNote = required.failed ? ` Reintentando ${required.failed} elemento(s).` : '';
        const errorNote = state.lastQueueError && Date.now() - state.lastQueueErrorAt < 10 * 60 * 1000
          ? ` Último inconveniente: ${state.lastQueueError}`
          : '';
        $('syncText').textContent = `Guardando el avance… ${required.count} elemento(s) pendiente(s)${eta > 1 ? `, aproximadamente ${Math.ceil(eta)} s` : ''}.${retryNote}${errorNote}`;
  ...
  async function updateClosingProgress(force) {
    ...
    if (percent === undefined) {
      const stats = await requiredProgressStats().catch(() => ({ total: 0, done: 0, pending: 1 }));
      percent = stats.total ? Math.min(92, Math.round((stats.done / stats.total) * 92)) : 0;
      if (!stats.pending && stats.total) percent = Math.max(percent, 94);
    }
