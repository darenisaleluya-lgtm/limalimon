(() => {
  ...
  async function initializeAccess() {
    ...
    const incomingToken = params.get('vt') || '';
    try {
  ...
  async function requestLocation() {
    ...
    return new Promise((resolve, reject) => {
      navigator.geolocation.getCurrentPosition(
        (position) => resolve({ latitude: position.coords.latitude, longitude: position.coords.longitude, accuracy: position.coords.accuracy, altitude: position.coords.altitude, altitudeAccuracy: position.coords.altitudeAccuracy, timestamp: position.timestamp }),
        () => reject(new Error('No fue posible autorizar la ubicación requerida. Revise los permisos del navegador.')),
  ...
  async function encryptBlob(blob, aad) {
    ...
    return { blob: new Blob([ciphertext], { type: 'application/octet-stream' }), ivB64: b64u(iv), aad, bytes: ciphertext.byteLength, mimeOriginal: blob.type || 'audio/webm' };
  }
  ...
  function renderQuestion() {
    ...
    $('questionCount').textContent = `${state.index + 1} / ${state.questions.length}`;
    ...
    if (q.type === 'radio' || q.type === 'select') {
      q.options.forEach((option) => {
        ...
        const input = document.createElement('input'); input.type = 'radio'; input.name = 'answer'; input.value = option; input.checked = current === option;
        const span = document.createElement('span'); span.textContent = option;
        label.append(input, span); area.appendChild(label);
      });
    } else if (q.type === 'number') {
      const input = document.createElement('input'); input.id = 'answerInput'; input.type = 'number'; input.value = current; area.appendChild(input);
    } else {
      const textarea = document.createElement('textarea'); textarea.id = 'answerInput'; textarea.value = current; textarea.placeholder = 'Escriba aquí…'; area.appendChild(textarea);
    }
  ...
  async function startQuestionRecorder(questionId, revision) {
    if (state.questionRecorder?.state !== 'inactive') await stopQuestionRecorder();
    const recorderStream = state.stream.clone();
    const context = { questionId, revision, parts: [], sequence: 0, mime: supportedMime() || 'audio/webm', recorderStream };
    ...
    recorder.ondataavailable = (event) => {
      ...
      state.questionChain = state.questionChain.then(async () => {
        const aad = `${state.sessionId}|question|${questionId}|${revision}|${part}`;
        const encrypted = await encryptBlob(event.data, aad);
        const id = `q|${state.sessionId}|${questionId}|${revision}|${part}`;
        const record = { id, sessionId: state.sessionId, kind: 'question', questionId, revision, part, totalParts: 0, uploaded: false, queued: false, ...encrypted, createdAt: Date.now() };
        await dbPut('encrypted', record);
        context.parts.push(id);
      });
    };
    recorder.onerror = () => recordEvent('QUESTION_RECORDER_ERROR', 'HIGH', questionId).catch(() => {});
  ...
  async function stopQuestionRecorder() {
    ...
    if (!recorder || !context) return context;
    ...
    if (context.recorderStream) context.recorderStream.getTracks().forEach((track) => track.stop());
    const total = context.parts.length;
    for (const id of context.parts) {
      const item = await dbGet('encrypted', id);
      if (item) { item.totalParts = total; await dbPut('encrypted', item); }
    }
  ...
  async function startFullRecorder() {
    ...
    recorder.ondataavailable = (event) => {
      ...
      state.fullChain = state.fullChain.then(async () => {
        ...
        await dbPut('encrypted', { id, sessionId: state.sessionId, kind: 'full', sequence, uploaded: false, queued: false, ...encrypted, createdAt: Date.now() });
  ...
  async function stopFullRecorder() {
    if (!state.fullRecorder) return;
    if (state.fullRecorder.state !== 'inactive') {
      const stopped = new Promise((resolve) => state.fullRecorder.addEventListener('stop', resolve, { once: true }));
      state.fullRecorder.stop();
      await stopped;
    }
    await state.fullChain;
    if (state.fullRecorder?._sourceStream) state.fullRecorder._sourceStream.getTracks().forEach((track) => track.stop());
  ...
  async function saveAndMove(direction) {
    ...
    try {
      ...
      for (const id of context.parts) {
        const item = await dbGet('encrypted', id);
        await enqueue({ action: 'uploadQuestionAudioPart', priority: 1, required: true, bytes: item.bytes, encryptedId: id, payload: { questionId: q.id, revision, part: item.part, totalParts: item.totalParts, ivB64: item.ivB64, aad: item.aad, mimeOriginal: item.mimeOriginal, bytes: item.bytes, clientTime: new Date().toISOString() } });
      }
      processQueue();
      if (direction === 'finish') {
        await beginClosing();
  ...
  async function serializeQueueItem(item) {
    ...
    if (item.encryptedId) {
      ...
      if (!encrypted) throw new Error('No se encontró el bloque cifrado local.');
      payload.ciphertextB64 = b64u(await encrypted.blob.arrayBuffer());
    }
    return payload;
  }
  ...
  async function queueEligibleFullItems() {
    ...
    const item = items[0];
    await enqueue({ action: 'uploadFullAudioPart', priority: 2, required: false, bytes: item.bytes, encryptedId: item.id, payload: { sequence: item.sequence, ivB64: item.ivB64, aad: item.aad, mimeOriginal: item.mimeOriginal, bytes: item.bytes, clientTime: new Date().toISOString() } });
    return true;
  }
  ...
  async function updateQueueUi() {
    ...
    if ($('syncText')) {
      if (!navigator.onLine) $('syncText').textContent = 'Sin conexión. El avance permanece protegido en este dispositivo.';
      else if (required.count) $('syncText').textContent = `Guardando tu avance… ${required.count} elemento(s) pendiente(s)${eta > 1 ? `, aproximadamente ${Math.ceil(eta)} s` : ''}.`;
      else $('syncText').textContent = 'Avance actualizado.';
    }
  }
